...
pub async fn leaderboards(
    ...
    Ok(Json(LeaderboardsResponse {
        top_earners_24h: state.db.top_earners_24h(state.config.leaderboard_limit).await?,
        top_payers_24h: state.db.top_payers_24h(state.config.leaderboard_limit).await?,
